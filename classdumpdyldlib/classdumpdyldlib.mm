		...
		for (unsigned d=0; d<protocolCount; d++){
			...
			if (writeToDisk){
				if (simpleHeader){
					...
					printf("-- %s \n", class_getImageName((Class _Nullable)protocol));
					imageOfProtocol=([imagePrefix isEqual:protocolPrefix] || !class_getImageName((Class _Nullable)protocol) ) ? imageName : [NSString stringWithCString:class_getImageName((Class _Nullable)protocol) encoding:NSUTF8StringEncoding];
...
extern "C" NSString * dumpBundle(NSBundle *aBundle){ 
	...
	NSString *savePath=[[NSFileManager defaultManager] isWritableFileAtPath:@"/tmp"] ? @"/tmp" : [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject];
	NSString *outputDir=[NSString stringWithFormat:@"%@/%@",savePath,[[[aBundle bundlePath] lastPathComponent] stringByDeletingPathExtension] ?: NSStringFromClass([aBundle principalClass])];
	NSError *error=NULL;
	BOOL isDir=NO;
	[[NSFileManager defaultManager] fileExistsAtPath:outputDir isDirectory:&isDir];
	if (!isDir){
